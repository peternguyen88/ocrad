console.log('ICO Transaction v33 - Auto 2.2 - Update ID - Send Captcha');
...
$('#div-buy-uch').on('hidden.bs.modal', function (e) {
    $('input[name="amount--coin--2ver3"]').val('');
    $('input[name="amount--uch--2ver3"]').val('');
});
...
var ICOTransaction = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.Result == "OK") {
                    ...
                    self.postCaptcha(data.Data);                                                                         //// ---- Auto Post Captcha
                }
                else {
    ...
    this.transferCoin = function (type) {
        if (type == 1) {
            var total_coin = parseFloat($('input[name="amount--coin--2ver3"]').val().trim());
            ...
            $('input[name="amount--uch--2ver3"]').val(amount);
        }
        else {
            var total_coin = parseFloat($('input[name="amount--uch--2ver3"]').val().trim());
            var __price = self.Blockchain == "BTC" ? parseFloat(self.Price().btc_last_price).toFixed(2) : parseFloat(self.Price().eth_last_price).toFixed(2);
            var __priceICO = Number(parseFloat(self.ICO().Price).toFixed(8));
            var amount = parseFloat(total_coin * __priceICO / __price).toFixed(8);
            $('input[name="amount--coin--2ver3"]').val(amount);
        }
    };
    this.buyAll = function () {
        ...
        $('input[name="amount--coin--2ver3"]').val(total_coin);
        $('input[name="amount--uch--2ver3"]').val(amount);
    };
        ...
        $.ajax({
            ...
            success: function (data) {
				...
				if($('#img-new-captcha').length && $('#img-new-captcha').first().attr('src')){
					self.postCaptcha($('#img-new-captcha').first().attr('src'));
				}
    ...
    this.Buy = function () {                                                     //// ---- Remove all validation to make sure that request will be sent to server, also make it run faster
        ...
        var captcha = realCaptcha ? realCaptcha : $('#input-captcha').val();
        
        var amount = $('input[name="amount--uch--2ver3"]').val();
        var coinPaid = $('input[name="amount--coin--2ver3"]').val();    
            ...
            success: function (data) {
                if (data.Result == "OK") {
                    if (data.Data.IsSuccess) {
                        ...
                        self.OpenBuyICO();      // Reload the Popup and buy again
    ...
    this.hackInputRobotCaptcha = function(){     // Submit Robot Captcha When Enter
        $('input[name="robot-check"]').keydown(function (event) {
                var keypressed = event.keyCode || event.which;
                if (keypressed == 13) {
                   self.correctRoboCaptcha();
                }
        });
        $('input[name="robot-check"]').focus();
    };
    
    this.hackInputCaptcha = function(){     // Submit Robot Captcha When Enter
    ...
    this.SetBuyMax = function(){
        ...
        $('input[name="amount--uch--2ver3"]').val(amountToSet);
        self.transferCoin(2);
    }
    	...
    	$.ajax({
    		...
    		success: function (data) {
    			...
    			if(data.solution && data.solution.text){
    				...
    				realCaptcha = data.solution.text;
