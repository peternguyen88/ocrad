console.log('ICO Transaction v36 - Auto 3.1 - Death By Captcha!');
...
var ICOTransaction = function () {
    ...
    this.HandleSubmissionError = function (message){                                        //// ---- If ICO not started - retry. If captcha invalid - reload captcha
        ...
        if(message && message.toLowerCase().indexOf('timed out') !== -1){
    ...
    this.resolveMappedCaptcha = function(imgSrc){
        ...
        console.log(imgSrc.substring(imgSrc.lastIndexOf('/')+1));
             ...
             if(imgSrc.indexOf('-350.png') !== -1) mappedCaptcha = '2u9zh';
        ...
        if(mappedCaptcha){
            ...
            if(self.ICO().BuyICO){	// Submit immediately if ICO Started
                ...
                var delayTime = 1250;
                console.log('Elapsed Time: ' + (new Date().getTime() - startTime)/1000+'; Delay Time: ' + delayTime);
	...
	this.setReloadTimeout = function(diffTime){                                                             //// ---- Refresh 5 mins before buy time to avoid auto logout
		...
		else if(diffTime && diffTime > 180){
			console.log('Set auto Reload Window after: '+Math.floor(diffTime - 180)+'s');
			setTimeout(function(){window.location.reload();}, Math.floor(diffTime - 180) * 1000);
		}
	};
            ...
            success: function (data) {
                ...
                if(!resolvedCaptcha.length){
                    ...
                    if(!antiCaptchaDecoded){
                        $('#input-captcha').val(trimCaptcha(resolvedCaptcha));
                        self.Buy();
                    }
                }
            },
